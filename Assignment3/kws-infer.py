...
def get_args():
    ...
    parser.add_argument("--n-mels", type=int, default=42)
    parser.add_argument("--win-length", type=int, default=None)
    parser.add_argument("--hop-length", type=int, default=164)
...
if __name__ == "__main__":
    ...
    if validators.url(args.checkpoint_url):
        checkpoint = args.checkpoint_url.rsplit('/', 1)[-1]
        # check if checkpoint file exists
        if not os.path.isfile(checkpoint):
            torch.hub.download_url_to_file(args.checkpoint_url, checkpoint)

    elif os.path.exists(os.path.join(args.path, "checkpoints", "transformer-kws-best-acc.pt")):
        checkpoint = os.path.join("dataset", "checkpoints", "transformer-kws-best-acc.pt")

    else:
        # pt_url = ???
        # checkpoint = pt_url.rsplit('/', 1)[-1]
        # # check if checkpoint file exists
        # if not os.path.isfile(checkpoint):
    ...
    if not args.gui:
        ...
        mel = rearrange(mel, 'c h (p2 w) -> p2 (c h w)', p2=14)  
        mel = mel.unsqueeze(0)
